...
export const getProduct = async (id: string): Promise<MyGetProductQuery> => {
  const statement = gql`query GetProduct($id: ID!) {
        getProduct(id: $id) {
          __typename
          id
          productId
  ...
  const data = await issueGQL(statement, 'GetProduct', gqlAPIServiceArguments);
  return data.getProduct as MyGetProductQuery;
};
